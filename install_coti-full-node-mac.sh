...
FULLNODE_INSTALLER_MAC_URL="${FULLNODE_INSTALLER_MAC_URL:-https://raw.githubusercontent.com/coti-io/coti-full-node/coti-mainnet/install_coti-full-node-mac.sh}"
...
: "${CLONE_BRANCH:=coti-mainnet}"
: "${NETWORK:=mainnet}"
: "${NGINX_ENABLED:=false}"
: "${FRPC_ENABLED:=false}"
: "${FRPS_SERVER_ADDR:=}"
: "${FRPS_SERVER_ADDR_1:=virginia.fullnode.mainnet.coti.io}"
: "${FRPS_SERVER_ADDR_2:=new-york.fullnode.mainnet.coti.io}"
...
if [[ ! "$FQDN" =~ ^[a-zA-Z0-9]([a-zA-Z0-9.-]*[a-zA-Z0-9])?$ ]] || [ "${#FQDN}" -gt 253 ]; then
    ...
    _w "Example: node1.fullnode.mainnet.coti.io"
    exit 1
fi
